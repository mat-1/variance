...
import { getWellKnown } from '../../../util/matrixUtil';
...
function Homeserver({ onChange }) {
  ...
  const setupHsConfig = async (servername: string) => {
    ...
    const wellKnown = await getWellKnown(servername);
    const baseUrl = wellKnown['m.homeserver'].base_url;
    const slidingSyncProxyUrl = wellKnown['org.matrix.msc3575.proxy']?.url;
    ...
    Promise.allSettled([tempClient.loginFlows(), tempClient.register()])
      .then((values) => {
        ...
        if (searchingHs !== servername) return;
        onChange({ baseUrl, slidingSyncProxyUrl, login: loginFlow, register: registerFlow });
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        } = await fetch(configFileUrl, { method: 'GET' }).then((res) => res.json());
...
function Login({ loginFlow, baseUrl }) {
  ...
  const submitter = async (values, actions) => {
    ...
    if (typeIndex === 0 && mxIdMatch) {
      ...
      userBaseUrl = (await getWellKnown(userBaseUrl))['m.homeserver'].base_url;
    }
